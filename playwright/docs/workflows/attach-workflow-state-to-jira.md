...
This workflow triggers **automatically**, immediately after any Workflow State Checkpoint sets `stage: "done"` with `remainingRepairs: 0`. This happens at several points depending on which workflow got there:

- `postman-generation.md` (V1) — right after generation, since V1 has no run/repair loop.
- `run-and-validate.md` / `run-existing-spec.md` — when a test run has no failures.
- `self-heal.md` — once every entry in `failures` reaches `status: "resolved"` (end of the single-failure workflow, or end of Phase 2 of the multi-failure triage).
...
1. Ask the user:
   ...
   > **Would you like the completed workflow state attached to a Jira ticket?** (Yes / No)
...
4. Once the ticket key is confirmed, run:
   ...
   ```
   playwright/scripts/attach-workflow-state-to-jira.sh <TICKET_KEY>
   ```
...
- Do not re-offer this once `stateAttachedToJira` is `true` for the current completed state — if the user starts a new workflow instance afterwards (new ticket/endpoint or new `specFile`/`scope`), the flag naturally resets since it's part of a fresh state file.
