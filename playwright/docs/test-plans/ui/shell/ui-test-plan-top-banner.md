...
| Bookmaker checkboxes | `getByTestId('user-menu-bookmaker-checkbox-<slug>')`, e.g. `user-menu-bookmaker-checkbox-bet365` | One per tracked bookmaker (23 total, industry-led UK list), in API-returned order (`Bet365`, `Betfair`, `BetUK`, `Ladbrokes`, `PaddyPower`, `SkyBet`, `WilliamHill`, `Coral`, `BetVictor`, `Betfred`, `EightEightEightSport`, `Unibet`, `LiveScoreBet`, `BoyleSports`, `VirginBet`, `Betway`, `ThirtyTwoRed`, `GrosvenorSport`, `QuinnBet`, `Spreadex`, `MansionBet`, `StarSports`, `CasumoBet` per the `Bookmaker` enum in `apps/api/openapi/user-config.yaml`), labeled with the display name via `formatBookmakerLabel()`; `slug` is the bookmaker's lowercase, non-alphanumeric-stripped name (e.g. `paddypower`, `williamhill`, `eighteighteightsport`). **The single remaining checked checkbox becomes `disabled` (in addition to staying `checked`) once it is the only enabled bookmaker**, preventing it from being unchecked at all — this was previously a bug (the checkbox visually flipped to unchecked despite the underlying state correctly staying enabled) but has now been fixed to disable the control outright, mirroring the equivalent fix already applied to the signup form's bookmaker checkboxes. |
...
| 11 | Functional | Bet Preferences section expands and loads current config | Open the dropdown, click the Bet Preferences "Configure" toggle | "Loading preferences..." appears briefly, then all bet-preference fields (odds format select, 23 bookmaker checkboxes, default bookmaker/bet-type selects, default stake input) become visible with values matching the current config |
| 12 | Functional | Enabled Bookmakers checkboxes reflect all 23 tracked bookmakers, all enabled by default for a new account | Sign up a brand-new account and open Bet Preferences for the first time | All 23 bookmaker checkboxes (`bet365`, `betfair`, `betuk`, `ladbrokes`, `paddypower`, `skybet`, `williamhill`, `coral`, `betvictor`, `betfred`, `eighteighteightsport`, `unibet`, `livescorebet`, `boylesports`, `virginbet`, `betway`, `thirtytwored`, `grosvenorsport`, `quinnbet`, `spreadex`, `mansionbet`, `starsports`, `casumobet`) are present, checked, and enabled (not disabled), since more than one bookmaker is enabled |
| 13 | Functional | Unchecking bookmakers down to the last one disables and locks it, keeping it checked | With Bet Preferences open, uncheck 22 of the 23 bookmaker checkboxes one at a time, leaving exactly one checked | After each uncheck, the remaining enabled bookmakers reflect correctly in the Default Bookmaker select; once only one bookmaker remains enabled, its checkbox is both `checked` and `disabled`, and a further click on it has no effect (checkbox stays checked, Default Bookmaker select still shows only that bookmaker) |
