...
| Enabled Bookmakers checkboxes | `getByTestId('bookmaker-checkbox-<Name>')` (e.g. `bookmaker-checkbox-Bet365`) | One per bookmaker (23 total, industry-led UK list): `Bet365`, `Betfair`, `BetUK`, `Ladbrokes`, `PaddyPower`, `SkyBet`, `WilliamHill`, `Coral`, `BetVictor`, `Betfred`, `EightEightEightSport`, `Unibet`, `LiveScoreBet`, `BoyleSports`, `VirginBet`, `Betway`, `ThirtyTwoRed`, `GrosvenorSport`, `QuinnBet`, `Spreadex`, `MansionBet`, `StarSports`, `CasumoBet` — all checked by default; hidden until "Configure" is clicked; at least one must remain checked. Locators/values use the raw enum token; visible labels are rendered via `formatBookmakerLabel()` (e.g. `PaddyPower` → "Paddy Power", `EightEightEightSport` → "888sport") |
...
| 3 | Functional | Betting Preferences sub-fields appear after clicking "Configure" | Click the "Configure" button | The four preference sub-fields become visible; the button label changes to "Hide"; all 23 bookmaker checkboxes are checked; Default Bookmaker select shows `Bet365` selected with all 23 bookmakers as options in order (`Bet365`, `Betfair`, `BetUK`, `Ladbrokes`, `Paddy Power`, `Sky Bet`, `William Hill`, `Coral`, `BetVictor`, `Betfred`, `888sport`, `Unibet`, `LiveScore Bet`, `BoyleSports`, `Virgin Bet`, `Betway`, `32Red`, `Grosvenor Sport`, `QuinnBet`, `Spreadex`, `MansionBet`, `Star Sports`, `Casumo Bet`); Default Bet Type select shows `Player Prop` selected with all 6 options in order (`Accumulator`, `Bet Builder`, `Player Prop`, `Superboost`, `FT Result`, `Other`); Default Stake input shows `5` |
