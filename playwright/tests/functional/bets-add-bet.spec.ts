...
test.describe('Add Bet Modal - Conditional Fields & Submission', () => {
  ...
  test('Functional - Selecting "Other" reveals the free-text Bet Type input', async ({
    page,
  }) => {
    ...
    await expect(
      modal.otherBetTypeInput,
      'Other Bet Type input should become visible',
    ).toBeVisible();
    await expect(
      modal.betTypeSelect,
      'Bet Type dropdown value should remain "Other"',
    ).toHaveValue('Other');
  });
  ...
  test('Functional - Selecting a multi-leg bet type swaps Fixture/Market/Player/Selection for the Legs editor', async ({
    ...
    await expect(
      modal.legsLabel,
      'Legs label should become visible',
    ).toBeVisible();
    await modal.legsEditor.expectLegCount(2);
    await expect(
      modal.legsEditor.legFixtureLabel(0),
      'Leg 0 Fixture label should be visible',
    ).toBeVisible();
    await expect(
      modal.legsEditor.legMarketLabel(0),
      'Leg 0 Market label should be visible',
    ).toBeVisible();

    await expect(
      modal.fixtureLabel,
      'Standalone Fixture label should be absent',
    ).toHaveCount(0);
    await expect(
      modal.marketLabel,
      'Standalone Market label should be absent',
    ).toHaveCount(0);
    await expect(
      modal.playerLabel,
      'Standalone Player label should be absent',
    ).toHaveCount(0);
    await expect(
      modal.selectionLabel,
      'Standalone Selection label should be absent',
    ).toHaveCount(0);
  ...
  test('Functional - Selecting "Bet Builder" renders the shared single-fixture picker instead of a per-leg fixture picker', async ({
    ...
    await expect(
      modal.legsEditor.legMarketLabel(0),
      'Leg 0 Market label should still be visible',
    ).toBeVisible();
  });

  test('Functional - "+ Add leg" adds a leg and "Remove" removes it (Cross Match Bet Builder)', async ({
    page,
  }) => {
    const betsPage = new BetsPage(page);
    await betsPage.openAddBetModal();
  ...
  test('Functional - Selecting a listed Fixture (Player Prop) reveals the Market field, hidden beforehand', async ({
    ...
    await expect(
      modal.marketLabel,
      'Market label should be absent before a fixture is selected',
    ).toHaveCount(0);
    ...
    await expect(
      modal.marketLabel,
      'Market label should become visible after a fixture is selected',
    ).toBeVisible();
    await expect(
      modal.marketSelect,
      'Market dropdown should become visible',
    ).toBeVisible();
  });
  ...
  test('Functional - Choosing "Other / not listed" in the Fixture dropdown reveals Home/Away Team inputs', async ({
    ...
    await expect(
      modal.homeTeamInput,
      'Home Team input should be absent before "Other / not listed" is chosen',
    ).toHaveCount(0);
    await expect(
      modal.awayTeamInput,
      'Away Team input should be absent before "Other / not listed" is chosen',
    ).toHaveCount(0);
    ...
    await expect(
      modal.homeTeamInput,
      'Home Team input should become visible',
    ).toBeVisible();
    await expect(
      modal.awayTeamInput,
      'Away Team input should become visible',
    ).toBeVisible();
    ...
    await expect(
      modal.homeTeamInput,
      'Home Team input should become absent again for a listed fixture',
    ).toHaveCount(0);
    await expect(
      modal.awayTeamInput,
      'Away Team input should become absent again for a listed fixture',
    ).toHaveCount(0);
  });

  test('Functional - Selecting a Market that requires a player reveals the Player field', async ({
    page,
  }) => {
    const betsPage = new BetsPage(page);
    await betsPage.openAddBetModal();
    ...
    await expect(
      modal.playerLabel,
      'Player label should become visible',
    ).toBeVisible();
    await expect(
      modal.playerSelect,
      'Player dropdown should become visible',
    ).toBeVisible();
  });
  ...
  test('Functional - Choosing "Other / not listed" in the Player dropdown reveals the manual Player input', async ({
    ...
    await expect(
      modal.playerManualInput,
      'Player manual text input should become visible',
    ).toBeVisible();
  });
  ...
  test('Functional - Selecting a Market with selections reveals the Selection (or combined Selection+Line) field', async ({
    ...
    await expect(
      modal.selectionLabel,
      'Selection label should be absent for a Yes-only market',
    ).toHaveCount(0);

    const optionLabels = await modal.marketSelect
      .locator('option:not([value=""])')
      .allTextContents();
    const nonYesOnlyLabel = optionLabels.find(
      (label) => !YES_ONLY_MARKETS.includes(label),
    );
    expect(
      nonYesOnlyLabel,
      'At least one non-Yes-only Player market should be available',
    ).toBeTruthy();
    await modal.marketSelect.selectOption({ label: nonYesOnlyLabel! });

    const selectionVisible = modal.selectionSelect.or(
      modal.selectionLineSelect,
    );
    await expect(
      selectionVisible,
      'Selection dropdown (or combined Selection+Line dropdown) should become visible for a non-Yes-only market',
    ).toBeVisible();
  });
  ...
  test('Functional - Stake Type "Normal + Free" swaps the single Stake field for separate Normal/Free Stake fields', async ({
    ...
    await expect(
      modal.stakeLabel,
      'Stake label should be visible by default',
    ).toBeVisible();
    ...
    await expect(
      modal.stakeLabel,
      'Stake label should become absent',
    ).toHaveCount(0);
    await expect(
      modal.normalStakeLabel,
      'Normal Stake label should become visible',
    ).toBeVisible();
    await expect(
      modal.normalStakeInput,
      'Normal Stake input should become visible',
    ).toBeVisible();
    await expect(
      modal.freeStakeLabel,
      'Free Stake label should become visible',
    ).toBeVisible();
    await expect(
      modal.freeStakeInput,
      'Free Stake input should become visible',
    ).toBeVisible();
    ...
    await expect(
      modal.stakeLabel,
      'Stake label should be restored',
    ).toBeVisible();
    await expect(
      modal.normalStakeLabel,
      'Normal Stake label should become absent again',
    ).toHaveCount(0);
    await expect(
      modal.freeStakeLabel,
      'Free Stake label should become absent again',
    ).toHaveCount(0);
  });

  test('Functional - Checking "Odds Boost?" reveals the Boost (%) field', async ({
    page,
  }) => {
    const betsPage = new BetsPage(page);
    await betsPage.openAddBetModal();
    ...
    await expect(
      modal.oddsBoostPercentLabel,
      'Boost (%) label should be absent by default',
    ).toHaveCount(0);
    ...
    await expect(
      modal.oddsBoostPercentLabel,
      'Boost (%) label should become visible',
    ).toBeVisible();
    await expect(
      modal.oddsBoostPercentInput,
      'Boost (%) input should become visible',
    ).toBeVisible();
    ...
    await expect(
      modal.oddsBoostPercentLabel,
      'Boost (%) label should become absent again',
    ).toHaveCount(0);
  });

  test('Functional - Selecting "Cashed Out" as Result reveals the Cash Out Value field', async ({
    page,
  }) => {
    const betsPage = new BetsPage(page);
    await betsPage.openAddBetModal();
    ...
    await expect(
      modal.cashOutValueLabel,
      'Cash Out Value label should be absent by default',
    ).toHaveCount(0);
    ...
    await expect(
      modal.cashOutValueLabel,
      'Cash Out Value label should become visible',
    ).toBeVisible();
    await expect(
      modal.cashOutValueInput,
      'Cash Out Value input should become visible',
    ).toBeVisible();
    ...
    await expect(
      modal.cashOutValueLabel,
      'Cash Out Value label should become absent again',
    ).toHaveCount(0);
  });
  ...
  test('Functional - Odds format preference toggles between decimal input and fractional numerator/denominator inputs', async ({
    ...
    await expect(
      ...
      'Decimal odds input should be visible by default',
    ).toBeVisible();
    await expect(
    ...
    await expect(
      betsPage.addBetModal.oddsNumeratorInput,
      'Odds numerator input should default to 1',
    ).toHaveValue('1');
    await expect(
      betsPage.addBetModal.oddsDenominatorInput,
      'Odds denominator input should default to 1',
    ).toHaveValue('1');
  });
  ...
  test('Functional - Submitting with invalid odds shows an inline error and does not close the modal', async ({
    ...
    await expect(
      modal.errorMessage,
      'Form error message should become visible',
    ).toBeVisible();
    await expect(
      modal.errorMessage,
      'Error message should mention valid decimal odds',
    ).toContainText('valid decimal odds');
  ...
  test('Functional - Successful submission with a concrete fixture shows the "Add another?" prompt; choosing "Yes" keeps the modal open, "No" closes it', async ({
    ...
    await expect(
      modal.addAnotherPrompt,
      '"Add another?" prompt should become visible',
    ).toBeVisible();
    await expect(
      modal.addAnotherHeading,
      'Prompt heading should read "Add another?"',
    ).toHaveText('Add another?');
    await expect(
    ...
    await expect(
      modal.addAnotherPrompt,
      'Prompt should hide after choosing "Yes"',
    ).toHaveCount(0);
    await expect(
      modal.modal,
      'Modal should remain open after choosing "Yes"',
    ).toBeVisible();
    await expect(
      modal.fixtureSelect,
      'Fixture selection should be retained after choosing "Yes"',
    ).not.toHaveValue('');
    ...
    await expect(
      modal.addAnotherPrompt,
      '"Add another?" prompt should become visible again',
    ).toBeVisible();
    await modal.addAnotherNoButton.click();

    await expect(
      modal.addAnotherPrompt,
      'Prompt should hide after choosing "No"',
    ).toHaveCount(0);
    await expect(
      modal.modal,
      'Modal should close entirely after choosing "No"',
    ).toHaveCount(0);
  });
});
